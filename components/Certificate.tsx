...
export default function Certificate({ onClose }: Props) {
  ...
  async function handleDonate() {
    if (!finalAmount || finalAmount < 1000 || donating) return;
    setDonating(true);
    try {
      const tossPayments = await loadTossPayments(CLIENT_KEY);
      const payment = tossPayments.payment({
        customerKey: `donor_${Date.now()}`,
      });
  ...
  function handlePrint() {
    ...
    <style>* { margin:0; padding:0; box-sizing:border-box; } body { background:#fff; display:flex; justify-content:center; align-items:center; min-height:100vh; font-family:'Apple SD Gothic Neo',sans-serif; } @media print { body { margin:0; } }</style>
  ...
  return (
    <div style={{
      ...
      <div ref={certRef} style={{
        ...
        background: "#fffdf5", border: "6px double #c8a96e",
        borderRadius: 4, padding: "32px 28px", textAlign: "center",
        fontFamily: "'Apple SD Gothic Neo', serif",
        boxShadow: "0 8px 40px #00000040", position: "relative",
      }}>
        {["top:4px;left:4px","top:4px;right:4px","bottom:4px;left:4px","bottom:4px;right:4px"].map((pos, i) => (
          <div key={i} style={{ position:"absolute", width:18, height:18, fontSize:14, lineHeight:"18px",
            ...Object.fromEntries(pos.split(";").map(p => p.split(":"))) }}>✦</div>
        ))}
        <div style={{ fontSize:28, marginBottom:4 }}>🏅</div>
        <p style={{ fontSize:11, color:"#a07840", letterSpacing:6, marginBottom:2 }}>CERTIFICATE OF AWARENESS</p>
        <h1 style={{ fontSize:28, fontWeight:900, color:"#1a1a1a", letterSpacing:4, marginBottom:16, borderBottom:"2px solid #c8a96e", paddingBottom:12 }}>상 장</h1>
        <div style={{ background:"#fff8e8", border:"1px solid #e8d5a0", borderRadius:8, padding:"12px 16px", marginBottom:16 }}>
          <p style={{ fontSize:13, color:"#a07840", marginBottom:6 }}>수 상 부 문</p>
          <p style={{ fontSize:20, fontWeight:900, color:"#1a1a1a", letterSpacing:2 }}>보이스피싱 예방 우수 체험자</p>
        </div>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:10, marginBottom:20, flexWrap:"wrap" }}>
          {["🚫 송금 거부 완료","🛡️ 범죄 예방 성공","✅ 체험 수료"].map(b => (
            <div key={b} style={{ background:"#f0f9e8", border:"1px solid #86c840", borderRadius:20, padding:"4px 12px", fontSize:11, color:"#3a7a00" }}>{b}</div>
          ))}
        </div>
        <p style={{ fontSize:12, color:"#aaa", marginBottom:8 }}>{today}</p>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
          <div style={{ height:1, flex:1, background:"#ddd" }} />
          <p style={{ fontSize:13, fontWeight:700, color:"#555", whiteSpace:"nowrap" }}>범죄예방 체험관</p>
          <div style={{ height:1, flex:1, background:"#ddd" }} />
        </div>
      ...
      {/* ── 후원 섹션 ── */}
      <div style={{ width:"100%", maxWidth:520, background:"#0d1a0d", border:"1px solid #22c55e44", borderRadius:20, padding:"20px 18px" }}>
        <p style={{ color:"#4ade80", fontWeight:900, fontSize:16, marginBottom:4 }}>💚 이 서비스를 후원해주세요</p>
          ...
          {DONATE_AMOUNTS.map(({ label, amount }) => (
            <button key={amount} onClick={() => { setSelectedAmount(amount); setCustomAmount(""); }}
              style={{
                ...
                background: selectedAmount === amount && !customAmount ? "#166534" : "#0a1a0a",
                color: selectedAmount === amount && !customAmount ? "#4ade80" : "#9ca3af",
                border: `1px solid ${selectedAmount === amount && !customAmount ? "#22c55e" : "#1a2a1a"}`,
              }}>
              <div>{label}</div>
        ...
        <button onClick={handleDonate}
          ...
          {donating ? "결제창 여는 중..." : `💳 ${finalAmount && finalAmount >= 1000 ? finalAmount.toLocaleString() + "원 후원하기" : "금액을 선택해주세요"}`}
        </button>

        <p style={{ color:"#374151", fontSize:10, textAlign:"center", marginTop:8 }}>
          🔒 토스페이먼츠 · 국내 전체 카드 · 해외카드 · 카카오페이 · 네이버페이 지원
        </p>
      </div>
      <div style={{ height:24 }} />
    </div>
